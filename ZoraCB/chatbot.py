...
input_answer = Input(shape=(maxlen_input,), dtype='int32', name='Answer')
...
merge_layer = concatenate([context_embedding, answer_embedding], name='concat')
out = Dense(int(dictionary_size/2), activation="relu", name='ReLU')(merge_layer)
out = Dense(dictionary_size, activation="softmax", name='SoftMax')(out)
...
def reply(que):
    ...
    text = preprocess(predout[0:start_index], '')
